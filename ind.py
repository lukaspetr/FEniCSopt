...
for setup in setups:
	...
	def phi(tau):
		...
		yh = Function(W)
		yh.vector()[:] = tau
		error = value_of_ind(V, cut_b_elem_dofs, bcs, epsilon, b, b_perp, c, f, yh)
	...
	rs.make_results('RESULTS/' + str(SC_EXAMPLE) + 'ind', NUM_CELL, V, W, uh, u_exact, yh, res_phi, results)

# Global results
rs.make_global_results('RESULTS/' + str(SC_EXAMPLE) + 'ind', global_results)
